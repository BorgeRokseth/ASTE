...
if __name__ == "__main__":
    ...
    initial_states_third_ship = SimulationConfiguration(
        initial_north_position_m=7048985.01,
        initial_east_position_m=246000.00,
        initial_yaw_angle_rad=45 * np.pi / 180,
    ...
    def grounding_index(list_of_lists, own_north, own_east):
        ''' Makes lists of distances to nearest ground, lists are reconstructed after loop with np.column_stack'''
        geo_own = geo.Point(own_north, own_east)

        for i in range(len(os.listdir(folder))):
            if i == 0:
                distance = geo_own.distance(enc.land.geometry)
                list_of_lists[i].append(distance)
                ...
                if distance < 50 and (i == 1 or i == 2 or i == 3 or i ==4):
    ...
    while own_ship.int.time <= own_ship.int.sim_time:
        ...
        if own_ship.int.time % 400 == 0:
            grounding_index(dist_ground, own_ship_north_position, own_ship_east_position)
